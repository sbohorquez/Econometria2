...
Podemos evitar hacer esta estimación no lineal, si hacemos test basándonos en la regresión de Gauss-Newton.
...
Con esto, el test consiste en probar la hipótesis nula de $b_p = 0$ en la regresión,
    \begin{equation}
        \tilde{u}_t = \mathbf{X}_t \boldsymbol{\beta} + b_p \tilde{u}_{t-1} + residuales
    \end{equation}
