...
\begin{align}
      ...
      Var(X_t) & = E[X^2_{t}]\\
              ...
              &= E[(\phi_1 x_{t-1}+...+\phi \rho x_{t-p}+ \epsilon_{t}- \theta_1 \epsilon_{t-1}-...- \theta_q \epsilon_{t-q})^2]\\
                ...
                &=\sum _{i=1}^p \phi^2_i E[x^2_{t-1}] +,E[\epsilon^2_t]+ \sum _{j=1}^q \theta^2_j E[\epsilon^2_{t-j}]\\

               &+2\sum _{i=1}^{^p} \sum _{j=1}^p\phi_i \phi_j E[x_{t-i} x_{t-j}]\\

              &+2 \sum _{i=1}^{^q} \sum_{j=1}^q \theta_i \theta_j E[\epsilon_{t-i} \epsilon_{t-j}] \text{para todo}  j\neq i\\

              &-2 \sum _{i=1}^q \theta_i E[\epsilon_{t} \epsilon_{t-i}]\\

              &- 2 \sum_{i=1}^p\phi_i E[\epsilon_{t} x_{t-i}]\\

              &- 2 \sum _{i=1}^{^p} \sum_{j=1}^q \phi_i \theta_j E[x_{t-i} \epsilon_{t-j}]\\
...
$=\sum _{i=1}^p \phi^2_i \gamma_0  +\sigma^2 + \sum _{j=1}^q \theta^2_j \sigma^2$

Tenemos presente que   $E[x_{t-i} x_{t-j}]=\gamma_{|i-j|}$ para todo $j \neq i$

$+2\sum _{i=1}^{^p} \sum _{j=1}^p\phi_i \phi_j\gamma_{|i-j|}]$
...
$+2 \sum _{i=1}^{^q} \sum_{j=1}^q \theta_i \theta_j E[\epsilon_{t-i} \epsilon_{t-j}]$ 
...
Si existen dos raíces iguales dentro del circulo unidad, digamos $\lambda_1 = \delta_q$ podemos expresar el procesos como un ARMA(p-1,q-1),
