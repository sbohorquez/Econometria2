...
\begin{itemize}
\item R cuadrado ajustado.
\item Criterio de Akaike (AIC): 
\begin{equation}
    AIC = -\log (MV)+2(k/T)
\end{equation}
\item Criterio de Scharwz (o criterio bayesiano) (BIC): 
\begin{equation}
    BIC = -\log (MV) + \ln (T)*K/T
\end{equation}
\end{itemize}

Cuando hay incongruencia entre el AIC y el BIC, es preferible el BIC porque castiga con la penalidad.
